...
export default function AdminDashboard({ metrics, recentOrders }: Props) {
  const cards = [
    { label: "Total Produk",       value: metrics.totalBooks.toString(),         icon: BookOpen,    color: "bg-blue-500",   change: "di database" },
    { label: "Total Pesanan",      value: metrics.totalOrders.toLocaleString(),   icon: ShoppingBag, color: "bg-amber-500",  change: "semua waktu" },
    { label: "Pendapatan Hari Ini",value: formatRupiah(metrics.todayRevenue),     icon: DollarSign,  color: "bg-green-500",  change: "hari ini" },
    { label: "Pengguna Aktif",     value: metrics.totalUsers.toLocaleString(),    icon: Users,       color: "bg-purple-500", change: "customer terdaftar" },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#6B7280] mt-0.5">{new Date().toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
        </div>
        <div className="flex gap-2">
          ...
          <Link href="/admin/products" className="flex items-center gap-1.5 px-4 py-2 border border-[#1E3A5F] text-[#1E3A5F] rounded-lg text-sm font-semibold hover:bg-[#1E3A5F] hover:text-white transition-colors">
            Kelola Produk
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {cards.map((card) => (
          <div key={card.label} className="bg-white rounded-xl p-5 shadow-sm">
            ...
            <p className="text-2xl font-semibold text-[#1F2937]">{card.value}</p>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <Link href="/admin/orders" className="text-sm text-[#1E3A5F] hover:underline flex items-center gap-1">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {recentOrders.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-[#6B7280]">Belum ada pesanan</td></tr>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <p className="text-sm text-[#6B7280] mb-2">Total Pendapatan</p>
          <p className="font-semibold text-[#1F2937]">{formatRupiah(metrics.totalRevenue)}</p>
          <p className="text-xs text-[#6B7280]">Dari semua pesanan selesai</p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <p className="text-sm text-[#6B7280] mb-2">Total Produk</p>
          <p className="font-semibold text-[#1F2937]">{metrics.totalBooks} Buku</p>
          <p className="text-xs text-[#6B7280]">Tersedia di database</p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <p className="text-sm text-[#6B7280] mb-2">Total Pelanggan</p>
          <p className="font-semibold text-[#1F2937]">{metrics.totalUsers} User</p>
          <p className="text-xs text-[#6B7280]">Terdaftar di sistem</p>
        </div>
      </div>
    </div>
  );
