    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl">
                ...
                <h3 className="text-base font-semibold text-[#1F2937] text-center mb-1">
                    Hapus produk ini?
                </h3>
...
export default function AdminProductList({
    ...
    const handleSearch = () => {
        router.get(
            "/admin/products",
            { search, category: category === "Semua" ? "" : category },
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-100">
                                <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B7280] uppercase w-10">
                                    No
                                </th>
                                ...
                                <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B7280] uppercase hidden lg:table-cell">
                                    Stok
                                </th>
                                <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B7280] uppercase">
                                    Status
                            ...
                            {books.data.length === 0 ? (
                                ...
                                books.data.map((book, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            <p className="text-sm font-semibold text-[#1F2937] max-w-[180px] line-clamp-2">
                                                {book.title}
                                            </p>
                                        ...
                                        <td className="px-4 py-3 text-sm font-medium text-[#1F2937] hidden sm:table-cell">
                                            {formatRupiah(book.price)}
                                        </td>
                                        <td className="px-4 py-3 text-sm text-[#1F2937] hidden lg:table-cell">
                                            {book.stock}
                                        </td>
                                        <td className="px-4 py-3">
                                            <span
                                                className={`text-xs font-semibold px-2 py-0.5 rounded-full ${book.in_stock ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                                            >
                                                {book.in_stock
                                                    ? "Aktif"
                                                    : "Nonaktif"}
                ...
                <div className="flex items-center justify-between px-5 py-3.5 border-t border-gray-100">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() =>
                                books.prev_page_url &&
                                router.visit(books.prev_page_url)
                            }
                        ...
                        {Array.from(
                            ...
                            <button
                                ...
                                className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${books.current_page === p ? "bg-[#1E3A5F] text-white" : "border border-gray-200 text-[#1F2937] hover:border-[#1E3A5F]"}`}
                            >
                                {p}
                            </button>
                        ))}
...
AdminProductList.layout = (page: React.ReactNode) => (
